...
export class CustomerComponent implements OnInit {
  ...
  displayedColumns: string[] = ['index', 'Name', 'Email', 'Mobile', 'Role', 'IsActive', 'Action'];
  ...
  constructor(private _router: Router, private _activatedRoute: ActivatedRoute, private readonly _commonService: CommonService,
    private readonly toast: ToastrService, private _userService: CustomerService) {
  ...
  OnActiveStatus(Id: number) {
    this._commonService.Question(Message.ConfirmUpdate as string).then(isTrue => {
      if (isTrue) {
        let subscription = this._userService.ChangeCustomerActiveStatus(Id).subscribe(
          data => {
  ...
  updateDeleteStatus(id: number) {
    ...
    this._commonService.Question(Message.ConfirmUpdate as string).then(result => {
      if (result) {
        let subscription = this._userService.DeleteCustomer(id).subscribe(
          data => {
            subscription.unsubscribe();
            if (data.IsSuccess) {
